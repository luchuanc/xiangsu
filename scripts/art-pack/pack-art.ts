...
import { packArtConfig } from "./ArtPackPacker";
...
export async function runPackCli(args: readonly string[] = process.argv.slice(2)): Promise<void> {
  ...
  await packArtConfig(config, { projectRoot: process.cwd(), mode: check ? "check" : "publish" });
  console.log(check ? "ART_PACK_CHECK_OK" : "ART_PACK_PACK_OK");
}
